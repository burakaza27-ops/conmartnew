...
interface BuyerNavProps {
  ...
  /** Field agents browse the catalog to raise assisted buyer requests. */
...
export function BuyerSidebarNav({ assistMode = false }: BuyerNavProps) {

  const { t } = useLanguage();



  const links = assistMode

    ? [

        { href: "/agent", label: t("agent_portal_back", "Agent portal"), icon: Briefcase, exact: true },
...
export function BuyerMobileBottomNav({
  ...
  const links = assistMode
    ...
    ? [
        ...
        { href: "/agent", label: t("agent_portal_back", "Agent"), icon: Briefcase, exact: true },
